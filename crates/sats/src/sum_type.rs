...
use crate::{static_assert_size, string, AlgebraicType, AlgebraicValue, SatsVec, SumTypeVariant};
...
impl From<SatsVec<SumTypeVariant>> for SumType {
    fn from(fields: SatsVec<SumTypeVariant>) -> Self {
        SumType::new(fields)
    }
}
...
impl<const N: usize> From<[(Option<&str>, AlgebraicType); N]> for SumType {
    fn from(fields: [(Option<&str>, AlgebraicType); N]) -> Self {
        fields.map(|(s, t)| SumTypeVariant::new(t, s.map(string))).into()
    }
}
