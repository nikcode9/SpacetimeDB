...
macro_rules! system_field {
    ($name:ident) => {
        ...
        impl From<$name> for NonEmpty<ColId> {
            fn from(value: $name) -> Self {
                NonEmpty::new(ColId(value as u32))
            }
        }
    };
}
...
impl StSequenceFields {
    pub fn name(&self) -> &'static str {
        match self {
            ...
            StSequenceFields::ColPos => "col_pos",
...
pub(crate) fn st_sequences_schema() -> TableSchema {
    TableDef::new(
        ...
        &[
            ...
            ColumnDef::sys(StSequenceFields::ColPos, AlgebraicType::U32),
...
impl<'a> TryFrom<&'a ProductValue> for StSequenceRow<&'a str> {
    ...
    fn try_from(row: &'a ProductValue) -> Result<StSequenceRow<&'a str>, DBError> {
        ...
        let col_pos = row.field_as_u32(StSequenceFields::ColPos as usize, None)?.into();
