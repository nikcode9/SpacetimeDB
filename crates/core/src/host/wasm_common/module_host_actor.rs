...
impl<T: WasmInstance> WasmModuleInstance<T> {
    ...
    fn call_reducer_with_tx(&mut self, tx: Option<MutTxId>, params: CallReducerParams) -> ReducerCallResult {
        ...
        let address = &dbic.address.to_abbreviated_hex();
        let reducer_name = &*self.info.reducers[reducer_id].name;
        REDUCER_COUNT.with_label_values(&[address, reducer_name]).inc();
        ...
        let reducer_span = tracing::trace_span!(
            "run_reducer",
            execution_duration = tracing::field::Empty,
        ...
        reducer_span
            .record("execution_duration", tracing::field::debug(execution_duration))
